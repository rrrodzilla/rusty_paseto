...
mod tests {
  ...
  #[test]
  fn verify_key_creation() {
    let key = Key::<32>::default();
    assert_eq!(key.len(), 32);
    let key = Key::<64>::default();
    assert_eq!(key.len(), 64);
    let key = Key::<24>::default();
    assert_eq!(key.len(), 24);
    let key_bytes = b"wubbulubbadubdubwubbalubbadubdub";
    let key = Key::<32>::from(key_bytes);
    assert_eq!(key.len(), 32);
  }

  #[test]
  fn verify_v1_local() -> Result<()> {
    //setup
    let key = Key::<32>::try_from("707172737475767778797a7b7c7d7e7f808182838485868788898a8b8c8d8e8f")?;
    let key = PasetoKey::<V1, Local>::from(&key);
    ...
    //verify
    assert_eq!(token, "v1.local.WzhIh1MpbqVNXNt7-HbWvL-JwAym3Tomad9Pc2nl7wK87vGraUVvn2bs8BBNo7jbukCNrkVID0jCK2vr5bP18G78j1bOTbBcP9HZzqnraEdspcjd_PvrxDEhj9cS2MG5fmxtvuoHRp3M24HvxTtql9z26KTfPWxJN5bAJaAM6gos8fnfjJO8oKiqQMaiBP_Cqncmqw8");

    //attempt to decrypt
    let parsed = Paseto::<V1, Local>::try_decrypt(&token, &key, None)?;
    assert_eq!(parsed, payload.as_ref());

    Ok(())
  }

  #[test]
  fn verify_v2_local() -> Result<()> {
    //setup
    let k = Key::<32>::try_from("707172737475767778797a7b7c7d7e7f808182838485868788898a8b8c8d8e8f")?;
    ...
    let json = json!({"data":"this is a signed message","exp":"2019-01-01T00:00:00+00:00"}).to_string();
    ...
    //attempt to decrypt
    let parsed = Paseto::<V2, Local>::try_decrypt(&token, &key, None)?;
    assert_eq!(parsed, payload.as_ref());
  ...
  fn verify_v1_public() -> Result<()> {
    ...
    //attempt
    let signature = Paseto::<V1, Public>::builder()
      .set_payload(payload)
      .try_sign(&private_key)?;

    //      //attempt to decrypt
    let verified = Paseto::<V1, Public>::try_verify(&signature, &public_key, None)?;
    assert_eq!(verified, payload.as_ref());

    Ok(())
  }
  #[test]
  fn verify_v2_public() -> Result<()> {
    //setup
    ...
    //attempt
    let signature = Paseto::<V2, Public>::builder()
      .set_payload(payload)
      .try_sign(&private_key)?;

    //verify
    assert_eq!(signature, "v2.public.eyJkYXRhIjoidGhpcyBpcyBhIHNpZ25lZCBtZXNzYWdlIiwiZXhwIjoiMjAxOS0wMS0wMVQwMDowMDowMCswMDowMCJ9HQr8URrGntTu7Dz9J2IF23d1M7-9lH9xiqdGyJNvzp4angPW5Esc7C5huy_M8I8_DjJK2ZXC2SUYuOFM-Q_5Cw");

    //attempt to decrypt
    let verified = Paseto::<V2, Public>::try_verify(&signature, &public_key, None)?;
    assert_eq!(verified, payload.as_ref());

    Ok(())
  }
  #[test]
  fn verify_v4_local() -> Result<()> {
    //setup
    let key = Key::<32>::try_from("707172737475767778797a7b7c7d7e7f808182838485868788898a8b8c8d8e8f")?;
    ...
    //attempt to decrypt
    let parsed = Paseto::<V4, Local>::try_decrypt(&token, &key, None, None)?;
    assert_eq!(parsed, payload.as_ref());
  ...
  fn verify_v4_public() -> Result<()> {
    ...
    let json = json!({"data": "this is a signed message","exp": "2022-01-01T00:00:00+00:00"}).to_string();

    let payload = Payload::from(json.as_str());

    //attempt
    let signature = Paseto::<V4, Public>::builder()
      .set_payload(payload)
      .try_sign(&private_key)?;

    //verify
    assert_eq!(signature, "v4.public.eyJkYXRhIjoidGhpcyBpcyBhIHNpZ25lZCBtZXNzYWdlIiwiZXhwIjoiMjAyMi0wMS0wMVQwMDowMDowMCswMDowMCJ9bg_XBBzds8lTZShVlwwKSgeKpLT3yukTw6JUz3W4h_ExsQV-P0V54zemZDcAxFaSeef1QlXEFtkqxT1ciiQEDA");

    //attempt to decrypt
    let verified = Paseto::<V4, Public>::try_verify(&signature, &public_key, None, None)?;
    assert_eq!(verified, payload.as_ref());
  ...
  fn verify_v3_local() -> Result<()> {
    ...
    //verify
    assert_eq!(token, "v3.local.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAADbfcIURX_0pVZVU1mAESUzrKZAsRm2EsD6yBoZYn6cpVZNzSJOhSDN-sRaWjfLU-yn9OJH1J_B8GKtOQ9gSQlb8yk9Iza7teRdkiR89ZFyvPPsVjjFiepFUVcMa-LP18zV77f_crJrVXWa5PDNRkCSeHfBBeg");

    //attempt to decrypt
    let parsed = Paseto::<V3, Local>::try_decrypt(&token, &key, None, None)?;
    assert_eq!(parsed, payload.as_ref());
